---
name: "ocp-configure"
description: "Configures the NVIDIA GPU Operator to use precompiled drivers on Red Hat CoreOS. Trigger keywords - GPU Operator, OpenShift, precompiled drivers, RHCOS, DTK, MIG, Multi-Instance GPU, A100, H100, partitioning, NVIDIA AI Enterprise, NVAIE, OCP, Virtualization, vGPU, GPU passthrough, KubeVirt, time-slicing, GPU sharing."
license: "Apache-2.0"
---

# Precompiled Drivers for the NVIDIA GPU Operator for RHCOS

<a id="gpu-operator-with-precompiled-drivers"></a>
...
- You know a CUDA version such as `12.1.0` that you want to use.
  ...
  One way to find a supported CUDA version for your operating system is to access the NVIDIA GPU Cloud registry at [CUDA | NVIDIA NGC](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/cuda/tags) and view the tags. Use the search field to filter the tags such as `base-ubi8` for RHEL 8 and `base-ubi9` for RHEL 9. The filtered results show the CUDA versions such as `12.1.0`, `12.0.1`, and `12.0.0`.
...
1. Clone the driver container repository:

   ```console
   $ git clone https://github.com/NVIDIA/gpu-driver-container.git
   ```

2. Change to the `rhel8/precompiled` directory under the cloned repository. You can build precompiled driver images for versions 8 and 9 of RHEL from this directory:

   ```console
   $ cd gpu-driver-container/rhel8/precompiled
   ```

3. Create a Red Hat Customer Portal Activation Key and note your Red Hat Subscription Management (RHSM) organization ID. These are to install packages during a build. Save the values to files such as `$HOME/rhsm_org` and `$HOME/rhsm_activationkey`:
...
4. Download your Red Hat OpenShift pull secret and store it in a file such as `${HOME}/pull-secret`:

   ```console
   export PULL_SECRET_FILE=$HOME/pull-secret.txt
   ```

5. Set the Red Hat OpenShift version and target architecture of your cluster such as `x86_64`:
...
6. Determine the Driver Toolkit (DTK) image for your target Red Hat OpenShift version and architecture:
...
7. Determine the RHEL and kernel versions of your target OpenShift cluster:
...
8. Set environment variables for the driver and CUDA versions, as well as the image:
...
Do not set the `DRIVER_TYPE`. The only supported value is currently `passthrough`, and this is set by default.
...
4. Check the **usePrecompiled** checkbox.

5. Specify values for **repository**, **version**, and **image**.
...
1. Use the procedure {ref}`create-cluster-policy-web-console` to create a cluster policy. Switch to the YAML view while creating the `ClusterPolicy` resource.

2. Add precompiled driver image properties:
...
3. Provide values for `image`, `repository`, and `version`. For example:
...
1. Create a template for the `ClusterPolicy` resource. Replace the NVIDIA GPU operator version with your value:
...
2. Modify the `clusterpolicy.json` file to specify values for `driver.usePrecompiled`, `driver.repository`, `driver.image` and `driver.version`. For example:
...
3. Create a `ClusterPolicy` resource from the modified `clusterpolicy.json` file:
...
4. Confirm that the driver container pods are running:
...
1. Disable precompiled driver support by modifying the cluster policy:
...
- **[references/mig-ocp.md](references/mig-ocp.md)** — Configure Multi-Instance GPU (MIG) partitioning for NVIDIA GPUs on OpenShift.
- **[references/nvaie-with-ocp.md](references/nvaie-with-ocp.md)** — Install and configure NVIDIA AI Enterprise on Red Hat OpenShift Container Platform.
- **[references/openshift-virtualization.md](references/openshift-virtualization.md)** — Enable GPU passthrough and vGPU support with the NVIDIA GPU Operator and OpenShift Virtualization.
- **[references/time-slicing-gpus-in-openshift.md](references/time-slicing-gpus-in-openshift.md)** — Configure GPU time-slicing to share NVIDIA GPUs among multiple workloads on OpenShift.
