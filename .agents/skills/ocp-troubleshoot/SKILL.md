...
description: "Diagnoses and resolve common issues with the NVIDIA GPU Operator on OpenShift. Trigger keywords - GPU Operator, OpenShift, troubleshooting, debug, DTK, Driver Toolkit, cleanup, uninstall, remove, appendix, reference."
license: "Apache-2.0"
---

# Troubleshooting GPU Operator on OpenShift

<a id="troubleshooting-gpu-ocp"></a>
...
1. Verify that the Node Feature Discovery has been created.
...
2. Ensure that there are nodes with GPUs. In this example, the check is performed for the NVIDIA GPU that uses the PCI ID `10de`.
...
1. Verify that the Custom Resource Definition (CRD) is deployed.
...
- **[references/clean-up.md](references/clean-up.md)** — Remove the NVIDIA GPU Operator and related resources from an OpenShift cluster.
- **[references/appendix-ocp.md](references/appendix-ocp.md)** — Supplemental reference material for GPU Operator on OpenShift.
