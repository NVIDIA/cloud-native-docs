...
For OpenShift Container Platform bare metal or VMware vSphere with GPU Passthrough no particular actions are needed. The default cluster policy works. For more information on creating a cluster policy see, :ref:`Create the cluster policy for the NVIDIA GPU Operator<create-cluster-policy-22.9.0>`.
