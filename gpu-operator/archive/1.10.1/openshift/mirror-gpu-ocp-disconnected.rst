...
.. _mirror-gpu-ocp-disconnected-1.10.1:
...
* A working OpenShift cluster up and running with a GPU worker node. See, `OpenShift Container Platform installation <https://docs.openshift.com/container-platform/latest/installing/installing-mirroring-installation-images.html>`_ for guidance on installing OpenShift Container Platform.
  ...
  .. note:: If installing the **NVIDIA GPU Operator** on OpenShift Container Platform version <``4.9.9`` you need to carry out the steps highlighted as **Optional** below. For more information see :ref:`broken driver toolkit <broken-dtk-1.10.1>`.
...
Follow the guidance :ref:`here <install-nfd-1.10.1>` to install the **Node Feature Discovery (NFD) Operator**. If you are installing on any Openshift Container Platform version other than ``4.8.19``, ``4.8.21`` or ``4.9.8`` proceed to :ref:`install-gpu-noworkaround-1.10.1`.
...
You can now proceed to :ref:`install and verify the NVIDIA GPU Operator <install-nvidiagpu-1.10.1>`.

.. _install-gpu-noworkaround-1.10.1:
